...
| 事实 | 数值 / 结论 | 出处 |
|---|---|---|
| 常驻 detach 后单进程私有内存 | **76.94 MB**（判据 2 达标；一期靶子 215.47 MB）。**三期重量（1.18.1）：73.42 / 75.95 MB**，与二期互印 | 二期矩阵格 B + 三期 Task 8 |
| 关窗销毁后 main 自持私有内存 | ~~**130.26 MB**（判据 1 未达 ≤60）~~ → **三期重量：87.82–89.62 MB（稳态）**。**130.26 是拆除瞬态、本轮未复现**；判据 1（≤60 MB）已注销（地板反证：空载 Electron 浏览器进程即 69.89–72.70 MB）。新靶子 = 地板 + 20 MB（本机 ≈90–93 MB）+ 三条取样纪律，详见 `2026-09-21-gateway-lite-mode-design.md` §一 | 二期矩阵格 A（memtree 按 root pid 自持行取值）+ 三期 Task 8 |
| 导航 8 样本 main 序列 | 二期：`129.01, 136.40, 141.31, 134.88, 136.25, 135.29, 132.92, 132.10` → 一期 121.32→147.10 单点不成立为持续爬升。**三期重量（1.18.1，两次）**：`111.56,116.12,121.75,114.68,115.27,115.76,115.69,116.14` 与 `108.75,115.96,120.46,114.88,111.95,112.43,111.84,112.42` —— 两次**峰值都在第 3 站、其后 5 站横盘不超峰**，形态成立；但二期那条「上升 ≤4 次」的阈值偏紧（实测 5/7 与 4/7），判读应以「峰值是否被后续超越」为准 | 二期判据 3 + 三期 Task 9 |
| 泡机 WAL | 50 条 HTTP 失败请求后 **1,388,472 B**，t+3→t+30min 恒定不动。**三期已定性为探针卫生事故**（兄弟探针按镜像名宽杀连坐打死泡机的常驻子进程），见本文件 §六 第 6 条与 `task-4-report.md` | 二期判据 4 + 三期 Task 4 |
| **泡机 WAL 的更正（三期 Task 4 取证）** | 上面那条「恒定不动」**不是产品缺陷，是探针卫生事故**：泡机与批次④其余探针并行，后者按镜像名宽杀 `taskkill /F /IM AgentHub.exe /T`，而泡机的常驻网关子进程**正是 AgentHub.exe** ⇒ 开跑几分钟后被连坐打死，此后泡的是**死进程**。Task 4 用改名后的唯一镜像名逐字节复现：宽杀后子进程死、日志断在 `detach-keep` 后一行不增、`-wal` 冻结在 1,388,472 B、主库停在 4096 B 空壳，与 `D:\tmp-agenthub-wal4` 现场**同形**；负对照（不发宽杀）里 tick 照常收敛。 | `task-4-report.md` + 物证 `D:\tmp-agenthub-wal4`（只读） |
...
2. **一条预期先红的闸**（红的是没收敛这件事本身，不再预设它是事务泄漏）：`scripts/dev-wal-convergence-test.cjs`（**新文件**，与 `dev-write-ownership-test` 分开——那条测写权，这条测收敛）。步骤：起子进程 → 走真实 HTTP 打 50 条**注定失败**的请求 → 调 `checkpoint()` → 断 WAL ≤64KB 且留痕字段可见。按 §二 微测指向，它现在应该红；红即是三期第一个缺陷的证据。若它直接绿，则冻结另有其因（候选：credits 后台作业的读事务），当场改判并写偏差。
   > **三期实测更正（Task 3 + Task 4）**：本闸实测**四条腿全绿**（进程内 / 非常驻真子进程 / detached+`--persistent`+Electron 运行时的判定腿 / 并发流量），二期那种冻结**今天在任何生产装配里都复现不出来**。二期那条「泡机 WAL 恒定不动」经 Task 4 取证为**探针卫生事故**（见 §二 更正行与本文件 §六 末条），**不是产品缺陷**。因此本闸对原缺陷的防护力 ≈ 0：它守不住一个复现不出来的状态；当前价值是可观测面 + 交给 Task 4 的最小冻结构造（Leg C：外部持读事务 ⇒ `ok:true` 且 `after===before`）。**不要**把它读成「二期那个冻结已经有闸守着了」。
...
6. **判据 4 的转绿方式（Task 4 回写）：原缺陷被证伪，不是被修好。** 二期那次「冻结」经取证为探针卫生事故（并行兄弟探针按镜像名宽杀，见 §二 更正行）。Task 4 在**没有并发宽杀**的前提下重跑了正确版泡机（真常驻：detached + `--persistent` + Electron 运行时，`AGENTHUB_CHECKPOINT_MS=30000`），跨 3 个周期 tick 的实测序列为
   `t+0s: 152472 B` → `t+30s: 1388472 → 0` → `t+60s: 1248392 → 0` → `t+90s: 1244272 → 0`（**FINAL = 0 B ≤ 64 KB**），
   每条 tick 的日志原文（`{"ok":true,"before":N,"after":0,"err":"","ms":30000}`）留档于 `task-4-report.md`。
   ⇒ 持续写入 + 周期 checkpoint ⇒ WAL 收敛，**已证**；而二期的「不收敛」源于子进程当时已死。
   ⚠ **纪律**：真机批次不许与泡机并行发按镜像名的宽杀（`taskkill /F /IM AgentHub.exe /T`）。
