...
//
// ===== 二期现场已定性：**探针卫生事故**，不是产品缺陷（Task 4 取证，2026-09-24）=====
// 二期泡机（`tmp/gateway-probe/real-batch4-walsoak.cjs`）自述「泡机期间主代理跑便携格」，而批次④
// 其余探针按**镜像名**宽杀——`real-batch4-portable.cjs:80/101/104` 与 `real-batch4-matrix.cjs:198/203`
// 都发 `taskkill /F /IM AgentHub.exe /T`，而**泡机的常驻网关子进程正是 `AgentHub.exe`**。
// Task 4 用改名后的唯一镜像名（`wal4kp-<随机串>.exe`，绝不碰真名）逐字节复现了这个机制：
//   杀之前：50 条请求后 `-wal = 1,388,472 B`、主库 `4096 B` 空壳、日志止于 `detach-keep`；
//   发一次宽杀后：子进程死、日志**一行都不再增**、`-wal` 冻结在 1,388,472 B、主库仍是 4096 B
//   —— 与 `D:\tmp-agenthub-wal4\` 现场（主库 4096 B、`-wal` 1,388,472 B、日志断在 detach-keep 后）
//   **同形**。负对照（同夹具**不发**宽杀、跑同样时长）里 tick 照常收敛（1,388,472 → 0）。
// ⇒ 二期那 25 分钟泡的是一个**已被兄弟探针连坐打死的死进程**，其 WAL 自然不会收敛。
//   `D:\tmp-agenthub-wal4\` 是物证，只读引用、绝不删改。
// ⇒ **纪律**：真机批次不许与泡机并行发按镜像名的宽杀；泡机期间不得跑会 `taskkill /IM AgentHub.exe`
//   的兄弟探针（`tmp/` 下这类脚本只列不删，它们不入库）。
// ⚠ 因此**不要**去改 `store.cjs` 的 `checkpoint()` 调用姿态（读返回码 / PASSIVE 退档）——那是修一个
//   不存在的缺陷，会把 SQLite 的正确行为「修」掉。
//
